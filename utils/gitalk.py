...
class Gitalk:
    ...
    def get_posts(self):
        ...
        for entry in entries:
            ...
            uri = entry.find("atom:link[@href]", namespaces=atom_namespaces).attrib[
                "href"
            ]
            uri_hash = md5(("/" + uri.lstrip(self.pattern)).encode("utf-8")).hexdigest()
            posts.append(Post(uri, uri_hash, title, True))
        self.posts = posts

    def form_issues(self, post):
        issue = {
            "title": f"Comments of {post.title}",
            "body": post.uri,
    ...
    def create_issue(self, issue):
        ...
        req = request.Request(
            url, data=json.dumps(issue).encode(), headers=headers, method="POST"
        )
